...
#define DEFAULT_NUM_THREADS 800 // Reduced from 1200
...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        if (elapsed > 240 && elapsed % 30 == 0) { // Shift every 30s after 240s
            targetPort += (rand() % 50); // Wider range
        }
        ...
        if (threadId == 0 && elapsed % 30 == 0) { // Log every 30s
