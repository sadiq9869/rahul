...
void packetSender(int threadId, const std::string& targetIp, int targetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    std::cout << CYAN << "Thread " << threadId << ": Sent " << threadPackets
              << " packets (" << std::fixed << std::setprecision(2) << threadDataMB << " MB)" << RESET << "\n";
...
int main(int argc, char* argv[]) {
    std::cout << MAGENTA << R"(
===============================================
   WELCOME TO ROHAN & SADIQ PACKET STORM TOOL
   DM to buy at: @Rohan2349 | @Sadiq9869
   Unleash the full power of UDP floods
===============================================
)" << RESET;
    ...
    std::cout << YELLOW << "Starting receiver thread...\n" << RESET;
    ...
    std::cout << GREEN << "[BRUST MODE] Sending packets to " << targetIp << ":" << targetPort
              << " for " << durationSeconds << " seconds | Packet Size: " << packetSize
              << " | Threads: " << numThreads << "..." << RESET << "\n";
    ...
    std::cout << YELLOW << "Waiting for receiver thread to join...\n" << RESET;
    receiverThread.join();
...
=======================================
    ...
    std::cout << CYAN << "Total Packet Size: " << totalPacketsSent << " packets\n";
    std::cout << "Total Data: " << std::fixed << std::setprecision(2) << totalDataMB << " MB\n";
    std::cout << "Contact for more: @Rohan2349 | @Sadiq9869" << RESET << "\n";

    return 0;
}

// @MADE BY YOUR FATHER - @Sadiq9869 & @Rohan2349
