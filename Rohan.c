...
" << RESET;
    ...
    if (totalPacketsReceived > 0) {
        std::cout << GREEN << "[+] Stats collected:
"
                  << "   Packets received: " << totalPacketsReceived << "
"
                  << "   Data received: " << totalDataMB << " MB
"
                  << "   Errors: " << totalErrors << RESET << "
";
    } else {
        std::cout << RED << "[!] Unable to read detailed stats (0 packets received).
"
                  << "    Possible reasons: no response, blocked port, or timeout." << RESET << "
";
    }
std::cout << YELLOW << "[*] Waiting for receiver thread to finish...\n" << RESET;
    receiverThread.join();
