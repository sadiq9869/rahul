...
#define FAKE_ERROR_DELAY 1000  // Increased to 1000ms for long duration stability
...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        int burstSize = (elapsed > 300) ? 3 : 5;  // Reduce burst size after 300s for stability
        ...
        for (int burst = 0; burst < burstSize && keepSending; burst++) {
            ...
            if (bytesSent > 0) {
                ...
                if (threadId == 0) {
                    std::cerr << "Thread " << threadId << ": Send failed, retry 1, errno: " << errno << " at " << elapsed << "s\n";
                }
                break;
            }
        }
