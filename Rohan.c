...
#define TARGET_PING_DELAY 677  // Force 677ms delay per packet
...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    while (keepSending) {
        ...
        if (bytesSent > 0) {
            ...
            totalPacketsSent++;
            std::this_thread::sleep_for(std::chrono::milliseconds(TARGET_PING_DELAY));  // Force 677ms delay
        } else if (bytesSent < 0) {
            threadFailures++;
            if (threadId == 0) {
        ...
        if (threadId == 0 && elapsed % 30 == 0) {
            std::cerr << "Thread 0 at " << elapsed << "s: " << threadPackets << " packets sent\n";
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Target Ping Forced to ~677ms per packet!\n";
    std::cout << "DM to buy at - @Rohan2349\n";
    return 0;
}