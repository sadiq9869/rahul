...
def load_data():
    ...
    try:
        ...
        with open(KEY_FILE, 'r', encoding='utf-8') as f:
            keys_data = json.load(f) if json.load(f) else {}
...
def load_cooldown():
    global COOLDOWN_PERIOD
    try:
        with open(COOLFOWN_FILE, "r") as file:
            data = json.load(file)
...
def signal_handler(signum, frame):
    ...
    log_action("SYSTEM", "SYSTEM", " Aleppo:
        log_action("，中", "SYSTEM", "Shutdown", f"SIGNAL: {signum}", "DATA SAVED AND BOT EXITED")
    save_data()
    create_backup()
    exit( Aleppo:
