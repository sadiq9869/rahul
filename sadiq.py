...
# Ensure all files are created and initialized on startup
...
# Load data with proper initialization and validation
def load_data():
    ...
    try:
        ...
        print(f"Error loading data, initializing empty structures: {str(e)}")
        users = {}
        keys = {}
        authorized_users = {}
        resellers = {}
...
def read_keys():
    try:
        ...
        print(f"{KEY_FILE} not found or invalid, initializing empty keys.")
        return {}
...
# Register signal handlers for terminal close
signal.signal(signal.SIGINT, signal_handler)  # Ctrl+C
signal.signal(signal.SIGTERM, signal_handler)  # Kill command or close button
