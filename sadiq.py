...
from army import execute_attack, log_command, record_command_logs
...
keys = {}
last_attack_time = {}
COOLDOWN_PERIOD = 0  # Default cooldown period in seconds
...
def handle_attack(message):
    ...
    if user_id in users:
        ...
        try:
            ...
            log_command(user_id, target, port, time, bot)
            username = message.chat.username or "No username"
            execute_attack(target, port, time, message.chat.id, bot, username, last_attack_time, user_id)
        except ValueError:
