...
COOLDOWN_PERIOD = 0  # Cooldown removed
RATE_LIMIT = 5
...
def escape_markdown(text):
    ...
    for char in special_chars:
        if char not in ['*', '_']:  # Allow emojis and bold/italic
...
def check_rate_limit(user_id):
    ...
    if len(command_timestamps[user_id]) >= RATE_LIMIT:
        if user_id not in admin_id and user_id not in permanent_admins:  # Only for non-admins
            return False
...
def execute_attack(target, port, time, chat_id, username, last_attack_time, user_id):
    ...
    try:
        ...
        send_response(message, f"❌ Error bhai! {str(e)}\n./Rohan check karo ya admin ko bolo! 👑")
...
def help_command(message):
    ...
    help_text = (
        ...
        "🔑 /genkey <duration> <devices> <keyname> - Key bana (e.g., 1day 2 sadiq) 🌍\n"
