...
# Utility function for safe replies with proper escaping
def safe_reply(bot, message, text):
    try:
        ...
        if "message to be replied not found" in str(e):
            ...
            # Log the error and retry without Markdown
            with open(os.path.join(DATA_DIR, "error_log.txt"), "a", encoding='utf-8') as log_file:
                log_file.write(f"[{datetime.datetime.now(IST).strftime('%Y-%m-%d %I:%M:%S %p')}] Markdown parse error: {e}, text={text}\n")
            bot.reply_to(message, text, parse_mode=None)  # Fallback to plain text
        else:
...
def execute_attack(target, port, time, chat_id, username, user_id):
    if active_attacks.get(user_id, False):
        if not is_admin(user_id, bot.get_chat(user_id).username):
            safe_reply(bot, message, "BSDK, ruk ja warna gaand mar dunga teri! Ek attack chal raha hai, dusra mat try kar!")
        else:
            safe_reply(bot, message, "Kripya karke BGMI ko tazi sa na choda! Ek attack already chal raha hai, wait karo.")
        return
    try:
...
- /genkey <duration> <key_name> - Generate key (admin/resellers)
...
EXAMPLES:
- /genkey 1d rahul
- /attack 192.168.1.1 80 1200
- /setcooldown 120
- /redeem Rahul_sadiq-rahul
Contact @Rahul_618 or join: t.me/devil_ddos
"""
...
def block_key(message):
    ...
    if key_name in keys:
        if keys[key_name].get("blocked", False):
            ...
            block_time = keys[key_name].get("blocked_time", "Unlocknown")
