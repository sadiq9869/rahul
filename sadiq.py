...
# Admin user IDs
admin_id = {"1807014348","898181945"}  # Replace with your admin Telegram ID
...
# Attack-related functions (from army.py)
def log_command(user_id, target, port, time):
    """
    Log attack details to a file.
    Args:
        user_id (str): User ID initiating the attack.
        target (str): Target IP address.
        port (int): Target port.
        time (int): Attack duration in seconds.
    """
...
def record_command_logs(user_id, command, target=None, port=None, time=None):
    """
    Record command logs to a file.
    Args:
        user_id (str): User ID.
        command (str): Command executed.
        target (str, optional): Target IP address.
        port (int, optional): Target port.
        time (int, optional): Attack duration in seconds.
    """
...
def execute_attack(target, port, time, chat_id, username, last_attack_time, user_id):
    """
    Execute an attack using the Rohan binary with fixed packet size and threads.
    Args:
        target (str): Target IP address.
        port (int): Target port.
        time (int): Attack duration in seconds.
        chat_id (int): Telegram chat ID for notifications.
        username (str): Username of the attacker.
        last_attack_time (dict): Dictionary tracking last attack times.
        user_id (str): User ID initiating the attack.
    """
    try:
