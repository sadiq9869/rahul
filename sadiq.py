    ...
    if user_id in users:
        try:
            ...
            if datetime.datetime.now(IST) < expiration_date:
                bot.reply_to(message, f"You already have an active key!\nYour access expires on: {expiration_date.strftime('%Y-%m-%d %I:%M:%S %p')}\nPlease wait until it expires to redeem a new key. To extend access, buy a new key for 50₹ after expiration and DM @{rahul_618}.", parse_mode='MarkdownV2')
                return
            else:
                del users[user_id]
                save_users()
        except ValueError:
            bot.reply_to(message, f"Error: Invalid expiration date format in your user data. Contact the admin: @{rahul_618}", parse_mode='MarkdownV2')
            return
    if key in keys:
        if keys[key].get("blocked", False):
            bot.reply_to(message, f"This key has been blocked. Contact the admin: @{rahul_618}", parse_mode='MarkdownV2')
            return
        ...
        hours, days, months = parse_duration(duration)
        if hours is None:
            bot.reply_to(message, f"Invalid duration in key. Contact the admin: @{rahul_618}", parse_mode='MarkdownV2')
            return
        ...
        bot.reply_to(message, f"Invalid or expired key! Buy a new key for 50₹. Make payment and DM @{rahul_618}.", parse_mode='MarkdownV2')
...
def handle_attack(message):
    ...
    if user_id in admin_id:
        ...
        try:
            ...
            has_access = False
    if not has_access:
        bot.reply_to(message, f"Unauthorized Access! Use /redeem <key_name> (e.g., /redeem Rahul_sadiq-rahul) or buy access for 50₹. Make payment and DM @{rahul_618} to get a key or ask an admin to authorize you.", parse_mode='MarkdownV2')
        return
