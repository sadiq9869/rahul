...
def load_data():
    ...
    initialize_system()
    max_retries = 3  # Limit retries to avoid infinite recursion
    retry_count = 0
        ...
        try:
            ...
            with open(KEY_FILE, 'r', encoding='utf-8') as f:
                ...
                for key_name, key_info in list(keys_data.items()):
                    if not key_info.get("blocked", False):
            ...
            if retry_count == max_retries:
                # If max retries reached, reset all files to default and log the failure
...
def generate_key(message):
    ...
    if is_overlord(user_id, username):
        if len(command_parts) < 3 or len(command_parts) > 4:
            response = "Usage for Overlord: /genkey <duration> [device_limit] <key_name>\nExample: /genkey 1d 999 sadiq or /genkey 1d all sadiq or /genkey 1d sadiq"
            safe_reply(bot, message, response)
            log_action(user_id, username, "/genkey", f"Command: {command}", response)
            return
    ...
    key_data = {
        ...
        "blocked_by_username": keys.get(custom_key, {}).get("blocked_by_username", ""),
        "blocked_time": keys.get(custom_key, {}).get("blocked_time", ""),
    ...
    safe_reply(bot, message, response)
    log_action(user_id, username, "/genkey", f"Key: {custom_key}, Duration: {duration}, Device Limit: {device_limit_display}, Generated by: @{generated_by}", response)
...
- /genkey - Generate a key
  - For Overlords: /genkey <duration> [device_limit] <key_name> (e.g., /genkey 1d 999 sadiq or /genkey 1d all sadiq)
  - For Admins/Resellers: /genkey key <duration> <key_name> (e.g., /genkey key 1d rahul)
...
- /balance quarrelsome - Check your reseller balance (resellers only)
- /myinfo - View your user information
- /listkeys - List all keys with detailed info (admin only)
EXAMPLE:
- /genkey 1d all sadiq - Generate a key with unlimited devices (Overlord only)
- /genkey 1d 999 sadiq - Generate a key with 999 device limit (Overlord only)
- /genkey key 1d rahul - Generate a 1-day key (Admins/Resellers)
...
def redeem_key(message):
    ...
    if user_id in users:
        try:
            ...
            response = "Error: Invalid expiration date format. Contact @Rahul_618"
            safe_reply(bot, message, response)
            log_action(user_id, username, "/redeem", f"Key: {key}", response, "Invalid expiration date format")
            return
    if key in keys:
        ...
        safe_reply(bot, message, response)
        log_action(user_id, username, "/redeem", f"Key: {key}, Expiration: {expiration_time.strftime('%Y-%m-%d %I:%M:%S %p')}", response)
    else:
...
def block_key(message):
    ...
    if key_name in keys:
        ...
        safe_reply(bot, message, response)
        log_action(user_id, username, "/block", f"Key: {key_name}, Blocked by: @{blocker_username}, Blocked on: {block_time}", response)
    else:
...
def add_user(message):
    ...
    save_data()
    response = f"User {target_user_id} added with access until {expiration_time.strftime('%Y-%m-%d %I:%M:%S %p')}"
    safe_reply(bot, message, response)
...
def handle_attack(message):
    ...
    if is_admin(user_id, username):
        ...
        try:
            ...
            has_access = False
    if not has_access:
        response = "BSDK, access nahi hai! Key le @Rahul_618 se ya admin se authorize karwa!"
        safe_reply(bot, message, response)
        ...
        if not v.get("blocked", False) and not is_expired:
            for device_id in v["devices"]:
                if device_id in users:
                    try:
                        user_expiration = datetime.datetime.strptime(users[device_id], '%Y-%m-%d %I:%M:%S %p').replace(tzinfo=IST)
...
def signal_handler(signum, frame):
    ...
    log_action("SYSTEM", "SYSTEM", "Shutdown", f"Signal: {signum}", "Data saved and bot exited")
    save_data()
    create_backup()
    exit(0)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            log_action("SYSTEM", "SYSTEM", "Bot Polling Error", f"Error: {str(e)}", "Retrying with backoff")
            print(f"Telegram API error: {e}")
            time.sleep(backoff_time)
            backoff_time = min(backoff_time * 2, 60)
        except Exception as e:
            log_action("SYSTEM", "SYSTEM", "Unexpected Error", f"Error: {str(e)}", "Retrying with backoff")
