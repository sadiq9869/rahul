...
def send_attack_finished_message(chat_id):
    ...
    try:
        ...
        print(f"Error sending attack finished message: {str(e)}")
...
def generate_key(message):
    ...
    if custom_key in keys:
        response = f"Key '{custom_key}' already exists. Please choose a different name."
        try:
    ...
    try:
        ...
        with open("error_log.txt", "a") as log_file:
            log_file.write(f"Error saving keys in generate_key at {datetime.datetime.now(IST)}: {str(e)}\n")
        bot.reply_to(message, "An error occurred while saving. Please contact the admin.")
        return

    # Escape special characters in generated_by and build response
    ...
    # Check response length and send
...
def help_command(message):
    ...
    help_text = (
        ...
        f"*EXAMPLES:*\n"
        f"- /genkey 1d rahul - Generate a 1-day key (e.g., Key: `Rahul_sadiq-rahul`)\n"
...
def check_balance(message):
    ...
    try:
        ...
        with open("error_log.txt", "a") as log_file:
            log_file.write(f"Error in check_balance at {datetime.datetime.now(IST)}: {str(e)}\nResponse: {response}\n")
        bot.reply_to(message, "An error occurred. Please contact the admin.")
...
def block_key(message):
    ...
    if key_name in keys:
        ...
        try:
            ...
            blocker_username = f"UserID_{user_id}"
...
def add_user(message):
    ...
    try:
        ...
        with open("error_log.txt", "a") as log_file:
            log_file.write(f"Error in add_user at {datetime.datetime.now(IST)}: {str(e)}\n")
        bot.reply_to(message, "An error occurred. Please contact the admin.")
    ...
    if all_users:
        for user, expiration in all_users.items():
            try:
                ...
                username = user_info.username if user_info.username else user_info.first_name or "Unknown"
                response += (
                    f"User ID: {user}\n"
                    f"Username: @{escape_markdown_v2(username)}\n"
...
def remove_reseller(message):
    ...
    try:
        ...
        with open("error_log.txt", "a") as log_file:
            log_file.write(f"Error in remove_reseller at {datetime.datetime.now(IST)}: {str(e)}\n")
        bot.reply_to(message, "An error occurred. Please contact the admin.")
            ...
            try:
                ...
                for device_id in devices:
                    try:
                        ...
                        username = f"Unknown (ID: {device_id})"
                    expiration = users.get(device_id, 'N/A')
                    device_details += f"  User ID: {device_id}\n  Username: @{escape_markdown_v2(username)}\n  Expiration: {expiration}\n"
                if not device_details:
                    device_details = "No users"
                ...
                response += f"Key: {key_name}\nDetails unavailable due to error: {str(e)}\n\n"
    else:
        response = "No keys found in keys\\.json\\."
    
    # Split response if too long for Telegram (max 4096 characters)
    max_message_length = 4000  # Safety margin
    if len(response) > max_message_length:
        parts = []
        current_part = ""
        for line in response.split("\n"):
            if line.strip() and len(current_part) + len(line) + 1 > max_message_length:
        ...
        for i, part in enumerate(parts):
            try:
                ...
                bot.send_message(message.chat.id, escape_markdown_v2(part), parse_mode='MarkdownV2')
            except Exception as e:
                with open("error_log.txt", "a") as log_file:
                    log_file.write(f"Error in list_keys (part {i+1}) at {datetime.datetime.now(IST).strftime('%Y-%m-%d %I:%M:%S %p')}: {str(e)}\nResponse: {part}\n")
                bot.send_message(message.chat.id, "An error occurred. Please contact the admin.")
    else:
        try:
            bot.send_message(message.chat.id, escape_markdown_v2(response), parse_mode='MarkdownV2')
        except Exception as e:
            with open("error_log.txt", "a") as log_file:
                log_file.write(f"Error in list_keys at {datetime.datetime.now(IST).strftime('%Y-%m-%d %I:%M:%S %p')}: {str(e)}\nResponse: {response}\n")
            bot.send_message(message.chat.id, "An error occurred. Please contact the admin.")
