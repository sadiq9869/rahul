...
def add_reseller(message):
    ...
    if len(command) != 3:
        bot.reply_to(message, "📜 *Usage*: `/add_reseller <user_id> <balance>`", parse_mode='MarkdownV2')
        return
...
def generate_key(message):
    ...
    command = message.text.split()
    if len(command) != 2:
        bot.reply_to(message, "📜 *Usage*: `/genkey <duration>`\n\n📋 *Available*: 1hour, 1day, 7days, 1month", parse_mode='MarkdownV2')
        return
...
def help_command(message):
    ...
    if user_id not in admin_id.keys():
        bot.reply_to(message, "🚫 *Access Denied*\n\n⚠️ This command is restricted to admins only\\.", parse_mode='MarkdownV2')
        return
    ...
    help_text = (
        ...
        "═══════════════════════════════\n"
        "🔥 *Bot Controls*\n"
        "  • `/start` - Launch the bot and enter the command center\\!\n"
        "  • `/help` - Display this powerful guide\\!\n"
        "  • `/status` - View bot uptime and system stats\\.\n"
        "\n"
        "⚡ *Power Management*\n"
        "  • `/attack [IP] [PORT] [TIME]` - Unleash a test attack\\! 🚀\n"
        "    Ex: `/attack 127.0.0.1 8000 60`\n"
        "  • `/add_reseller <user_id> <balance>` - Add a new reseller\\! 🔥\n"
        "  • `/genkey <duration>` - Generate a VIP key\\! 🛠️\n"
        "  • `/logs` - View recent logs and records\\! 📂\n"
        "  • `/users` - List authorized users\\! ⚔️\n"
        "  • `/remove <user_id>` - Remove a user\\! 🚷\n"
        "  • `/resellers` - View reseller details\\! 🎖️\n"
        "  • `/addbalance <reseller_id> <amount>` - Add balance to a reseller\\! 💎\n"
        "  • `/remove_reseller <reseller_id>` - Remove a reseller\\! ⚰️\n"
        "\n"
        "💡 *Example Commands*\n"
        "  • `/attack 127.0.0.1 8000 60` - Launch a 60-second attack\\!\n"
        "  • `/genkey 1day` - Create a 1-day VIP key\\! ⏳\n"
        "═══════════════════════════════\n"
        "💬 *Need help\\? Contact an Admin*: @Rohan2349, @sadiq9869"
    )
...
def start_command(message):
    ...
    markup.add(attack_button, myinfo_button, redeem_button, status_button)
    welcome_message = (
        "🌟 *WELCOME TO VIP DDOS PANEL* 🌟\n"
        "═══════════════════════════════\n"
        "🔥 *Unleash the Power Below*:\n"
...
def run_attack(user_id, ip, port, duration, chat_id):
    ...
    try:
        ...
        # Threads are hardcoded in Rohan.c as DEFAULT_NUM_THREADS (800)
...
def remove_user(message):
    ...
    command = message.text.split()
    if len(command) != 2:
        bot.reply_to(message, "📜 *Usage*: `/remove <User_ID>`", parse_mode='MarkdownV2')
        return
...
def add_balance(message):
    ...
    if user_id in admin_id.keys():
        ...
        if len(command_parts) != 3:
            bot.reply_to(message, "📜 *Usage*: `/addbalance <reseller_id> <amount>`", parse_mode='MarkdownV2')
            return
...
def remove_reseller(message):
    ...
    if user_id in admin_id.keys():
        ...
        if len(command_parts) != 2:
            bot.reply_to(message, "📜 *Usage*: `/remove_reseller <reseller_id>`", parse_mode='MarkdownV2')
            return
