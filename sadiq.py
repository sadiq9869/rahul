...
def generate_key(message):
    ...
    if minutes is None and hours is None and days is None and months is None:
        bot.reply_to(message, "Invalid duration. Use formats like 30min, 1h, 1d, 1m, 1hours, 1days, 1months")
        return
    ...
    if minutes > 0:
        ...
        cost_duration = "1m" if months == 1 else f"{month}m"
    elif days > 0:
        cost_duration = "1d" if days == 1 else f"{day}d"
    elif hours > 0:
        cost_duration = "1h" if hours == 1 else f"{hour}h"

    # Generate custom key name
    custom_key = f"Rahul_sadiq-{key_name}"

    # Generate or update key
    ...
    # Update existing users' expiration if they redeemed this key
