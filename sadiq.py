...
def help_command(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ Error sending message. Contact support.", parse_mode='MarkdownV2')
...
def start_command(message):
    ...
    welcome_message = (
        ...
        "  • 💰 View balance\n"
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📲 Owner: {escape_markdown_v2('@Pk_Chopra')}\n"
        f"💬 Support: {escape_markdown_v2('@Rohan2349')}, {escape_markdown_v2('@sadiq9869')}"
    )
    try:
...
def handle_attack_button(call, user_id):
    if user_id in users:
        ...
        if datetime.datetime.now() > expiration_date:
            bot.send_message(call.message.chat.id, f"❌ Access expired \\| Contact {escape_markdown_v2('@Rohan2349')} or {escape_markdown_v2('@sadiq9869')}\\.", parse_mode='MarkdownV2')
            return
        ...
        bot.send_message(call.message.chat.id, f"⛔ Unauthorized \\| Contact {escape_markdown_v2('@Pk_Chopra')}, {escape_markdown_v2('@Rohan2349')}, or {escape_markdown_v2('@sadiq9869')}\\.", parse_mode='MarkdownV2')
...
def handle_attack_command(message):
    ...
    if user_id not in users:
        bot.reply_to(message, f"⛔ Unauthorized \\| Contact {escape_markdown_v2('@Pk_Chopra')}, {escape_markdown_v2('@Rohan2349')}, or {escape_markdown_v2('@sadiq9869')}\\.", parse_mode='MarkdownV2')
        return
    expiration_date = datetime.datetime.strptime(users[user_id], '%Y-%m-%d %H:%M:%S')
    if datetime.datetime.now() > expiration_date:
        bot.reply_to(message, f"❌ Access expired \\| Contact {escape_markdown_v2('@Rohan2349')} or {escape_markdown_v2('@sadiq9869')}\\.", parse_mode='MarkdownV2')
        return
    if user_id in last_attack_time:
        time_since_last_attack = (datetime.datetime.now() - last_attack_time[user_id]).total_seconds()
        if time_since_last_attack < COOLDOWN_PERIOD:
    ...
    try:
        ...
        response = (
            ...
            "━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📲 Powered by: {escape_markdown_v2('@Rohan2349')} & {escape_markdown_v2('@sadiq9869')}"
        )
...
def list_authorized_users(message):
    ...
    for user, expiration in users.items():
        ...
        response += f"👤 ID: {user}\n📛 Username: {escape_markdown_v2('@' + username if username else user_info.first_name)}\n⏳ Expires: {expiration}\n\n"
    try:
        bot.reply_to(message, response, parse_mode='MarkdownV2')
    except telebot.apihelper.ApiTelegramException as e:
        bot.reply_to(message, "❌ Error sending message. Contact support.", parse_mode='MarkdownV2')
...
def show_resellers(message):
    ...
    for reseller_id, balance in resellers.items():
        ...
        response += f"👤 Username: {escape_markdown_v2('@' + username if username != 'Unknown' else username)}\n🆔 ID: {reseller_id}\n💰 Balance: {balance} Rs\n\n"
    try:
        bot.reply_to(message, response, parse_mode='MarkdownV2')
    except telebot.apihelper.ApiTelegramException as e:
        bot.reply_to(message, "❌ Error sending message. Contact support.", parse_mode='MarkdownV2')
...
def status_command(message):
    ...
    response += f"━━━━━━━━━━━━━━━━━━━━━━━\n📲 Powered by: {escape_markdown_v2('@Rohan2349')} & {escape_markdown_v2('@sadiq9869')}"
    try:
        bot.reply_to(message, response, parse_mode='MarkdownV2')
    except telebot.apihelper.ApiTelegramException as e:
        bot.reply_to(message, "❌ Error sending message. Contact support.", parse_mode='MarkdownV2')
